...
from .utils import config_logic, case_logic, get_time_stamp,timestamp_to_datetime,add_suite_data,edit_suite_data,env_data_logic,get_total_values,upload_file_logic,task_logic
from .runner import run_test_by_type,run_by_single,run_by_batch
from .tasks import main_hrun,project_hrun
...
def test_run(request):
    ...
    if request.is_ajax():
        ...
        run_test_by_type(id, base_url, testcase_dir_path, type)
        # 最终把该测试用例的数据生成目录下的yml文件hat\suite\2020-06-02 15-13-31-748
        
        runner.run(testcase_dir_path) #运行该路径的yml文件 suite\2020-06-02 15-13-31-748\CAS\getuser\getuser_1.yml
        #shutil.rmtree(testcase_dir_path)
        summary = timestamp_to_datetime(runner._summary, type=False)  #runner._summary是运行结果
        #print(summary)
...
def task_add(request):
    ...
    if request.is_ajax():
        ...
        project = Project.objects.all().order_by('-create_time')
        context_dict = {'project': project, 'env': env}
        return render(request, 'task_add.html', context_dict)
