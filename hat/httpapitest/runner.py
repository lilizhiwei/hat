...
def run_by_single(index, base_url, path):
    ...
    include = eval(obj.include) #前置
    request = eval(obj.request) #请求信息
    name = obj.name  #用例名称
    project = obj.belong_project #所属项目
    module = obj.belong_module.module_name #所属模块
    ...
    if not os.path.exists(testcase_dir_path):
        os.makedirs(testcase_dir_path)  #不存在目录创建该目录
    ...
    #最终生成目录下的文件hat\suite\2020-06-02 15-13-31-748\CAS\getuser\getuser_1.yml
